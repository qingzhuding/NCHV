...
def GenStablizerList(RabiFre, freq1=200, freq2=207.6372):
    ...
    # phi2=Angle[i][j][1]*pi
    # phi1=Angle[i][j][3]*pi
    value = [[(freq1 * 2 * pi, Angle[i][j][2] / RabiFre / 2.0, Angle[i][j][3] * pi), (freq2 * 2 * pi, Angle[i][j]
                                                                                      [0] / RabiFre / 2.0, Angle[i][j][1] * pi)] for i in range(4) for j in range(3)]  # colmn and lines have Checked
...
def GenTomographList(RabiFre, freq1=200, freq2=207.6372):
    ...
    tomo_value = [[(freq1 * 2 * pi, tomo_angle[i][2] / RabiFre / 2.0, tomo_angle[i][3] * pi), (freq2 * 2 * pi,
                                                                                               tomo_angle[i][0] / RabiFre / 2.0, tomo_angle[i][1] * pi)] for i in range(8)]  # Please Check This Line
    return tomo_value
